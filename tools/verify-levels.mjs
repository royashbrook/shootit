...
for (let n = 1; n <= LEVEL_COUNT; n++) {
  const t0 = Date.now()
  const { level, salt, bot } = levelFor(n)
  const ms = Date.now() - t0
  fingerprints.push(JSON.stringify([level.gates, level.packs, level.boss, bot.count, bot.t]))
...
for (let n = 1; n <= LEVEL_COUNT; n++) {
  ...
  if (JSON.stringify([level.gates, level.packs, level.boss, bot.count, bot.t]) !== fingerprints[n - 1]) {
